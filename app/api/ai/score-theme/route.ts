...
- vision : 達成したい理想状態が、応募者が共感できる解像度で描かれているか。「業界を変える」のような言い回しだけでなく、5年後/10年後にどんな景色が広がっているかが見えるか。
  さらに **必ず「その状態を表す目標数値」(定量的な事実) を含むこと**。例: 「○年までに県内30施設で導入率70%」「年商10億円」など。数値が無ければ大幅減点。
- current_state : ビジョンに対する現在の立ち位置が、観察可能な「事実」(数値・現場の声・行動データ) で書かれているか。
  **必ず定量的な事実 (数値・割合・件数・頻度等) を含むこと**。
  ※「うまくいっていない」「停滞している」のような主観・評価・推測は禁止 — それは vision と current_state のギャップ (=pain) を見て初めて判定できるもので、現状の記述ではない。事実だけを書く欄。
- pain : 理想 (vision) と現状 (current_state) の差分=ギャップが事実として記述されているか (問題=事実のズレ、課題=取り組むべきこと、と混同しない)。
  **必ず定量的なギャップ (vision の目標数値と current_state の現在値の差) を含むこと**。憶測・一般論・「不便だと思う」は減点。事実かどうかが鍵。
